...
def rainbow_plot_stereo(left, right, wid, filename):
    ...
    pylab.hold(False)
    pylab.savefig("%s.png" % filename, bbox_inches='tight', pad_inches=0, transparent=True)
...
def process(samplerate, waveform, filename):
    ...
    for i in range(start_time, end_time if end_time != -1 else seconds_in_z):
        ...
        # create our bins
        left_channel_bins = [sum(left_channel[j*frqstep:j*(frqstep+1)]) for j in range(int(lowfrq/frqstep), int(highfrq/frqstep))]
        right_channel_bins = [sum(right_channel[j*frqstep:j*(frqstep+1)]) for j in range(int(lowfrq/frqstep), int(highfrq/frqstep))]
        ...
        frq_dom = [j*frqstep for j in range(len(left_channel_bins))]
        ...
        rainbow_plot_stereo(left, right, 3, "%s.%s" % (filename, str(i)))
